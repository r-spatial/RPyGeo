...
RPyGeo is a package providing access to ArcGIS geoprocessing tools in R. It archives this by building an interface between R and the ArcPy Python side-package via the reticulate package. 
...
In this tutorial we use geographic data from the `spData` package and streamline the workflow with the `%>%` operator from the `magrittr` package. The `elev` and `nz` dataset are used to demonstrate raster and vector file operations.  
