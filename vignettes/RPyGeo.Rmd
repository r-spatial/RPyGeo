...
ArcPy is organized into modules which are Python files with functions and classes. The ArcPy main module `arcpy` (`arcgis`)has several hundred functions and classes. 
...
`rpygeo_search` returns all available algorithms that contain the search term (`search_term`) in their name. 
...
The query above will return all functions of the ArcPy main module with a name that contains the expression `3d`. 
If we want to search another module we have to change the `module` parameter.
...
For example, to compute the slope from the digital elevation model we have exported, we can type:

```{r, eval=FALSE}
arcpy$Slope_3d(in_raster = "elev.tif", out_raster = "slope.tif")
```

This created a file named `slope.tif` in the workspace directory, which in our case is the temporary folder (see above in `rpygeo_build_env()`).
To do that, run `rpygeo_load("slope.tif")` (see also furhter below). <!-- Marc, is that correct? -->
...
To use the functions of a specific extension, you have to first access the corresponding extension via the `$` operator, and then have to use a second `$`  to get access to its functions.
For example, to compute the slope from a digital elevation model with the help of the Spatial Analyst extenstion, we run:

```{r, eval=FALSE}
arcpy$sa$Slope(in_raster = "elev.tif")
```

Note that there is no output parameter, hence the result of the processing will be saved to the scratch workspace location (see again `rpygeo_build_env()`).
...
<!-- well, I would never write that... a right-handed assignment operator-->
```{r, eval=FALSE}
slope <- arcpy$Slope_3d(in_raster = "elev.tif", out_raster = "slope.tif") %>%
  rpygeo_load()
```

<!-- Marc: and is the same possible with arcpy$sa$Slope? -->
