...
To demonstrate both raster and vector operations we will use the raster object `dem` and the vector object `nz` from the packages **RQGIS** and **spData**.
...
writeRaster(dem, file.path(tempdir(), "dem.tiff"), format = "GTiff")
...
rpygeo_search(search_term = "3d", module = arcpy) %>%
  head
#> [1] "ASCII3DToFeatureClass_3d"        
#> [2] "AddFeatureClassToTerrain_3d"     
#> [3] "AddSurfaceInformation_3d"        
#> [4] "AddTerrainPyramidLevel_3d"       
#> [5] "AddZInformation_3d"              
#> [6] "Adjust3DZ_management"  
```
...
#> "ClassifyRaster" "MLClassify"     "Reclassify" 
```
...
Code completion (pressing Shift) will also list the available function parameters for a specific function, e.g. for `arcpy$Slope_3d()`.
